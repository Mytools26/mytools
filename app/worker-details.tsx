...
export default function WorkerDetailsScreen() {
  ...
  const location = workerTools.find((tool) => tool.location)?.location || "No location";
  const totalQuantity = workerTools.reduce((sum, tool) => sum + Number(tool.quantity || 0), 0);
  const brokenCount = workerTools.filter((tool) => tool.status === "Broken").length;
  ...
  const handleDelete = (toolId?: string, toolName?: string) => {
    if (!toolId) return;
    Alert.alert(t("deleteTool"), `Delete ${toolName}?`, [
      { text: t("cancel"), style: "cancel" },
      { text: t("delete"), style: "destructive", onPress: async () => { deleteTool(toolId); await cloudDeleteTool(toolId); } },
    ]);
  };

  const handleReturn = (toolId: string, toolName: string) => {
    Alert.alert(t("returnTool"), `Return ${toolName} to warehouse?`, [
      { text: t("cancel"), style: "cancel" },
      { text: t("return"), onPress: async () => { returnTool(toolId); await cloudReturnTool(toolId); } },
    ]);
  };

  const handleReturnAll = () => {
    if (workerTools.length === 0) return;
    Alert.alert(t("returnAll"), `Return all tools from ${workerNameText}?`, [
      { text: t("cancel"), style: "cancel" },
  ...
  const startEdit = (toolId: string, quantity: string) => { setEditingId(toolId); setNewQuantity(quantity || ""); };

  const saveEdit = async (tool: any, realId: string) => {
    updateTool(realId, { ...tool, id: realId, quantity: newQuantity });
    await cloudUpdateToolQuantity(realId, newQuantity);
    setEditingId(null);
    setNewQuantity("");
    Alert.alert(t("saved"), "Quantity updated");
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.heroCard}>
        <Text style={styles.heroTitle}>{t("workerOverview")}</Text>
        <Text style={styles.heroText}>{t("assignedTools")}</Text>
      </View>

      <View style={styles.statsRow}>
        <View style={styles.statCard}>
          <Text style={styles.statNumber}>{workerTools.length}</Text>
          <Text style={styles.statLabel}>{t("toolLines")}</Text>
        </View>
        <View style={styles.statCard}>
          <Text style={styles.statNumber}>{totalQuantity}</Text>
          <Text style={styles.statLabel}>{t("totalQty")}</Text>
        </View>
      </View>
      ...
      <View style={styles.statsRow}>
        <View style={styles.statCard}>
          ...
          <Text style={styles.statLabel}>{t("broken")}</Text>
        </View>
        <View style={styles.statCard}>
          <Text style={styles.warningNumber}>{missingCount}</Text>
          <Text style={styles.statLabel}>{t("missing")}</Text>
        </View>
      </View>

      <View style={styles.mainActions}>
        <TouchableOpacity style={styles.pdfButton} onPress={exportWorkerPdf}>
          <Text style={styles.mainButtonText}>{t("exportPdf")}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.returnAllButton} onPress={handleReturnAll}>
          <Text style={styles.mainButtonText}>{t("returnAll")}</Text>
        </TouchableOpacity>
      </View>

      <TouchableOpacity style={styles.addToolsButton} onPress={() => router.push({ pathname: "/add-tool", params: { workerName: workerNameText, location } } as any)}>
        <Text style={styles.mainButtonText}>{t("addMoreTools")}</Text>
      </TouchableOpacity>

      <Text style={styles.sectionTitle}>{t("assignedTools")}</Text>
          ...
          return (
            <View key={realId} style={styles.toolCard}>
              <View style={styles.toolTop}>
                <View style={styles.leftBox}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.toolMeta}>{tool.category || "General"} · {tool.brand || "No brand"}</Text>
                  </View>
                </View>
              ...
              <View style={styles.middleRow}>
                ...
                {isEditing ? (
                  <TextInput style={styles.quantityInput} value={newQuantity} onChangeText={setNewQuantity} keyboardType="numeric" />
                ) : (
                  <View style={styles.quantityBadge}>
                    <Text style={styles.quantityText}>x{tool.quantity || "0"}</Text>
                  </View>
                )}
              </View>
              {isEditing ? (
                <View style={styles.actionRow}>
                  ...
                  <TouchableOpacity style={styles.cancelButton} onPress={() => { setEditingId(null); setNewQuantity(""); }}>
                    <Text style={styles.buttonText}>{t("cancel")}</Text>
                  </TouchableOpacity>
                </View>
              ) : (
                <View style={styles.actionRow}>
                  <TouchableOpacity style={styles.editButton} onPress={() => startEdit(realId, tool.quantity || "")}>
                    <Text style={styles.buttonText}>{t("edit")}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.returnButton} onPress={() => handleReturn(realId, tool.name)}>
                    <Text style={styles.buttonText}>{t("return")}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.deleteButton} onPress={() => handleDelete(realId, tool.name)}>
...
const styles = StyleSheet.create({
  ...
  heroCard: { backgroundColor: "#ff6b00", borderRadius: 20, padding: 18, marginBottom: 18 },
  heroTitle: { color: "white", fontSize: 24, fontWeight: "bold" },
  heroText: { color: "white", fontSize: 14, marginTop: 4 },
  statsRow: { flexDirection: "row", gap: 10, marginBottom: 10 },
  statCard: { flex: 1, backgroundColor: "#111c34", borderRadius: 16, padding: 16, borderWidth: 1, borderColor: "#1f2937" },
  statNumber: { color: "#ff6b00", fontSize: 28, fontWeight: "bold" },
  statLabel: { color: "#9ca3af", fontSize: 12, marginTop: 4 },
  warningNumber: { color: "#fbbf24", fontSize: 28, fontWeight: "bold" },
  problemNumber: { color: "#f87171", fontSize: 28, fontWeight: "bold" },
  mainActions: { flexDirection: "row", gap: 10, marginTop: 8, marginBottom: 10 },
  pdfButton: { flex: 1, backgroundColor: "#2563eb", padding: 15, borderRadius: 14, alignItems: "center" },
  returnAllButton: { flex: 1, backgroundColor: "#16a34a", padding: 15, borderRadius: 14, alignItems: "center" },
  addToolsButton: { backgroundColor: "#ff6b00", padding: 15, borderRadius: 14, alignItems: "center", marginBottom: 22 },
  mainButtonText: { color: "white", fontSize: 15, fontWeight: "bold" },
  sectionTitle: { color: "white", fontSize: 22, fontWeight: "bold", marginBottom: 12 },
  emptyText: { color: "#9ca3af", fontSize: 16 },
  toolCard: { backgroundColor: "#111c34", borderRadius: 18, padding: 14, marginBottom: 12, borderWidth: 1, borderColor: "#1f2937" },
  toolTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  leftBox: { flexDirection: "row", alignItems: "center", flex: 1, marginRight: 10 },
  toolEmoji: { fontSize: 28, marginRight: 10 },
  toolName: { color: "white", fontSize: 17, fontWeight: "bold" },
  toolMeta: { color: "#9ca3af", fontSize: 12, marginTop: 4 },
  middleRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 14 },
  locationText: { color: "#d1d5db", fontSize: 13, flex: 1, marginRight: 10 },
  statusBadge: { borderRadius: 999, paddingHorizontal: 12, paddingVertical: 6 },
  statusText: { color: "white", fontSize: 11, fontWeight: "bold" },
  quantityBadge: { backgroundColor: "#020b1f", borderRadius: 999, paddingHorizontal: 12, paddingVertical: 7, borderWidth: 1, borderColor: "#374151" },
  quantityText: { color: "#ff6b00", fontSize: 14, fontWeight: "bold" },
  quantityInput: { backgroundColor: "#020b1f", color: "white", width: 74, padding: 9, borderRadius: 12, fontSize: 16, textAlign: "center", borderWidth: 1, borderColor: "#374151" },
  actionRow: { flexDirection: "row", gap: 8, marginTop: 14 },
  editButton: { flex: 1, backgroundColor: "#2563eb", paddingVertical: 10, borderRadius: 12, alignItems: "center" },
  returnButton: { flex: 1, backgroundColor: "#16a34a", paddingVertical: 10, borderRadius: 12, alignItems: "center" },
  deleteButton: { flex: 1, backgroundColor: "#7f1d1d", paddingVertical: 10, borderRadius: 12, alignItems: "center" },
  saveButton: { flex: 1, backgroundColor: "#16a34a", paddingVertical: 10, borderRadius: 12, alignItems: "center" },
  cancelButton: { flex: 1, backgroundColor: "#374151", paddingVertical: 10, borderRadius: 12, alignItems: "center" },
